...
library(dplyr)
x<-cs_rhizo10_df%>%group_by(Genotype)%>%summarise(mean=mean(CAP1))
y<-cs_rhizo10_df%>%group_by(Genotype)%>%summarise(mean=mean(CAP2))
x
y
library(ggplot2)
...
q<-p+geom_segment(aes(x = -0.156, y = -0.0623, xend = 0.115, yend = -0.186),
               arrow = arrow(length = unit(0.5, "cm")))
r<-q+geom_segment(aes(x = -0.156, y = -0.0623, xend = 0.161, yend = 0.139),
                  arrow = arrow(length = unit(0.5, "cm")))
r
#Add the segements as arrows for centers of Col-0, msa and sdi
#center for Col-0 (x=,y=)

ggsave(
  "CAP_rhiz10_with_arrows.tiff",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
